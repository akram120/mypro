...
const url = 'https://192.168.20.21/Services/WSRemitAPI.asmx?wsdl';
    ...
    parser.parseString(requestXmlFromInternalSystem , function(err, result){
     ...
     if ((serviceInfo.service_type).toUpperCase() == "Q_REM") {
        getRemitDetails(xmlRequestToMusharbash).then(result => {
            var yesResponseJSON = result['soap:Envelope']['soap:Body'];
            var builder = new xml2js.Builder();
            if('ResponseCode' in yesResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']){
                var yesResponseXML = builder.buildObject(result);
                console.log(remitNO);
                var responseToInernalSystem = writeGeneralErrorQuery(yesResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseMessage'],yesResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseCode'],no_server_error);
                ...
                newData.save(async (err, doc) => {
                    if (!err) {
                        ...
                        var responseToInernalSystemE = writeGeneralErrorQuery(yesResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseMessage'],yesResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseCode'],database_error);
                        callback(responseToInernalSystemE);
                    }
                });
            } else {
                    var detailedResponse = yesResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult'];
                    var yesResponseXML = builder.buildObject(result);
        ...
        findQResponse(remitNO).then(value=> {
            payRemit(xmlRequestToMusharbash).then(result => {
                var yesResponseJSON = result['soap:Envelope']['soap:Body']['PayRemitResponse']['PayRemitResult'];
                var builder = new xml2js.Builder();
                var yesResponseXML = builder.buildObject(result);
                var responseToInernalSystem = writeConfirmPayResponse(yesResponseJSON,no_server_error);
                let newData = new newApiRequest.insertData(
                    {
                        rem_no: remitNO,
                        service_name: serviceInfo.service_name,
                        ...
                        Request: requestXmlFromInternalSystem,
                        remStatus:yesResponseJSON['ResponseCode']==0?"1":""
                    });
                console.log(newData);
                ...
                newData.save(async (err, doc) => {
                    if (!err) {
                        ...
                        console.log("DataBase Error");
                        var responseToInernalSystemE = writeConfirmPayResponse(yesResponseJSON,database_error);
                        callback(responseToInernalSystemE);
                    }
                });
            }).catch(err=>{
...
async function getRemitDetails(xml) {
    ...
    const { response } = await soapRequest({ url: "http://100.0.0.108:3150/musharbash/query", headers: header, xml: xml });
...
async function payRemit(xml) {
            ...
            const { response } = await soapRequest({ url: "http://100.0.0.108:3150/musharbash/pay", headers: header, xml: xml });
            const { headers, body, statusCode } = response;
            console.log(body);
            console.log(statusCode);
