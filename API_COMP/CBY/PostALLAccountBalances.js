...
const PostALLAccountBalancesEndPoint = "/Transaction/PostALLAccountBalances";
...
router.post('/CBY/PostALLAccountBalances', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await PostALLAccountBalancesToCBY(formattedRequest);
...
async function PostALLAccountBalancesToCBY(req) {
    ...
    const response = await PostALLAccountBalances(token, req);
...
async function PostALLAccountBalances(token, req) {
