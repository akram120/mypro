...
const PostTrialBalanceEndPoint = "/Transaction/PostTrialBalance";
...
router.post('/CBY/PostTrialBalance', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await PostTrialBalanceToCBY(formattedRequest);
...
async function PostTrialBalanceToCBY(req) {
    ...
    const response = await PostTrialBalance(token, req);
...
async function PostTrialBalance(token, req) {
