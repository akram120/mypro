...
const sendRemitEndPoint = "/Transaction/SendRemit";
...
router.post('/CBY/SendRemit', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await sendRemitToCBY(formattedRequest);
...
async function sendRemitToCBY(req) {
    ...
    const response = await sendRemit(token, req);
...
async function sendRemit(token, req) {
    console.log(token+'<--token');
    const options = {
        method: 'POST',
        uri: `${baseURL}${sendRemitEndPoint}`,
        headers: {
