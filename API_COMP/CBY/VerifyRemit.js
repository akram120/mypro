...
const VerifyRemitEndPoint = "/Transaction/VerifyRemit";
...
router.post('/CBY/VerifyRemit', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await VerifyRemitToCBY(formattedRequest);
...
async function VerifyRemitToCBY(req) {
    ...
    const response = await VerifyRemit(token, req);
...
async function VerifyRemit(token, req) {
    console.log(token+'<--token');
    const options = {
        method: 'PUT',
        uri: `${baseURL}${VerifyRemitEndPoint}`,
        headers: {
