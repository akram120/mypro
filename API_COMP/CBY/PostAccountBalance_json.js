...
const PostAccountBalanceEndPoint = "/AccountBalances/PostAccountBalance";
...
router.post('/CBY/PostAccountBalance', express.json(), async (req, res) => {
    try {
        ...
        const response = await PostAccountBalance(token, jsonRequests);
...
async function PostAccountBalance(token, requests) {
    const options = {
        method: 'POST',
    ...
    try {
        ...
        console.error('Error PostAccountBalance :', error);
    ...
    if (response.error /*&& response.error.length > 0*/) {
        for (let i = 0; i < response.error.length; i++) {
            ...
            const newData = new newApiRequest.insertData({
                ...
                transaction_id: "",
                service_name: "CBY",
                service_type: "PostAccountBalance",
                date: Date.now(),
