...
router.post('/CBY/SendAllRemit', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
...
async function SendAllRemitToCBY(req) {
    ...
    const response = await SendAllRemit(token, req);
...
async function SendAllRemit(token, req) {
    ...
    try {
        ...
        console.error('Error sendeing remit:', error);
        throw error; // إعادة رمي الخطأ حتى يمكن التعامل معه في مكان آخر
    }
...
function xmlToJson(xmlString) {
    ...
    parser.parseString(xmlString, (err, result) => {
        ...
        console.log(JSON.stringify(result, null, 2) + 'res');
    });
}
