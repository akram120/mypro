...
const PostAccountBalanceEndPoint = "/Transaction/PostAccountBalance";
...
router.post('/CBY/PostAccountBalance', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await PostAccountBalanceToCBY(formattedRequest);
...
async function PostAccountBalanceToCBY(req) {
    ...
    const response = await PostAccountBalance(token, req);
...
async function PostAccountBalance(token, req) {
    console.log(token+'<--token');
    const options = {
        method: 'PUT',
        uri: `${baseURL}${PostAccountBalanceEndPoint}`,
        headers: {
