...
const PostAccountCategoryIssuesEndPoint = "/TrialBalances/PostAccountCategoryIssues";
...
router.post('/CBY/PostAccountCategoryIssues', express.json(), async (req, res) => {
    try {
        ...
        const response = await PostAccountCategoryIssues(token, jsonRequests);
...
async function PostAccountCategoryIssues(token, requests) {
    const options = {
        method: 'POST',
    ...
    try {
        ...
        console.error('Error PostAccountCategoryIssues :', error);
    ...
    if (response.error /*&& response.error.length > 0*/) {
        for (let i = 0; i < response.error.length; i++) {
            ...
            const newData = new newApiRequest.insertData({
                ...
                transaction_id: "",
                service_name: "CBY",
                service_type: "PostAccountCategoryIssues",
                date: Date.now(),
