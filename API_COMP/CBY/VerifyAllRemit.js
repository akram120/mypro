...
const VerifyAllRemitEndPoint = "/Transaction/VerifyAllRemit";
...
router.post('/CBY/VerifyAllRemit', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await VerifyAllRemitToCBY(formattedRequest);
...
async function VerifyAllRemitToCBY(req) {
    ...
    const response = await VerifyAllRemit(token, req);
...
async function VerifyAllRemit(token, req) {
    console.log(token+'<--token');
    const options = {
        method: 'PUT',
        uri: `${baseURL}${VerifyAllRemitEndPoint}`,
        headers: {
