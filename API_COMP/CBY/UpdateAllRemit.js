...
router.post('/CBY/UpdateAllRemit', express.text({ type: 'application/xml' }), async (req, res) => {
    try {
        ...
        const response = await UpdateAllRemitToCBY(formattedRequest);
...
async function UpdateAllRemitToCBY(req) {
    ...
    const response = await UpdateAllRemit(token, req);
...
async function UpdateAllRemit(token, req) {
