...
const mainUrl = "https://uptuat3.aktifbank.com.tr/ISV/TU/WebServices/V1_4/CorpService.asmx?wsdl";
...
function upt_service(jsonObject, fullJson , callback)
{
  var js ; 
  ...
  if (((jsonObject['soapenv:Body'].service_info.service_type).toUpperCase() == Q_service_type))
  {
      ...
      getPayRemittanc(xmlReturned, name , function(jsonData,bodyReturned)
      {
          ...
          var Rem_no =  xpath.find(serverJson , '//tem:UPT_REF')[0] //serverJson['soapenv:Body']['tem:CorpGetTransferInfo']['tem:obj']['tem:UPT_REF']       
  ...
  else if ((jsonObject['soapenv:Body'].service_info.service_type).toUpperCase() == P_service_type)
  {
    ...
    getPayRemittanc(xmlReturned, name , function(jsonData,bodyReturned)
    {
        Rem_no = xpath.find(serverJson , '//tem:UPT_REF')[0]  ; //serverJson['soapenv:Body']['tem:CorpPaymentRequest']['tem:obj']['tem:UPT_REF']  
      let newData= new newApiRequest.insertData(
        {
          rem_no :Rem_no,
          transaction_id:Rem_no,
...
module.exports.upt_service = upt_service;
