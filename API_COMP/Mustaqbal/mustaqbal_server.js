...
const url = 'http://apinew.mustaqbalcom.com/Services/WSRemitAPI.asmx';
...
async function getRemitDetails(xml) {
    ...
    const { response } = await soapRequest({ url: "http://100.0.0.108:3150/mustaqbal/query", headers: header, xml: xml });
...
async function payRemit(xml) {
            ...
            const { response } = await soapRequest({ url: "http://100.0.0.108:3150/mustaqbal/pay", headers: header, xml: xml });
            const { headers, body, statusCode } = response;
            console.log(body);
            console.log(statusCode);
