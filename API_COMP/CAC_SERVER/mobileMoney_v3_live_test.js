  ...
  //check opration type 
            ...
            let newData= new newApiRequest.insertData(
              {
                ...
                FirstName:Res_Code == CoSuccessCode ? respones['CustomerName'] : '',
                SecondName:"",
                ThirdName:"",
                LastName:"",
                CustID:"",
    ...
    console.log(cashinData);
      ...
      else
      {
        var err_json={code:'00003', massege:'حدث خطاء اثناء الاتصال مع الخدمة الرجاء التواصل مع مدير النظام '};
        var object='{}';
        var err_msg_praper=writeSearchXmlFile(object,err_json,'حدث خطاء اثناء الاتصال مع الخدمة الرجاء التواصل مع مدير النظام ' );
         callback(err_msg_praper);
      }
       ...
       mmGetAccountDeposit(AccountDepositData,token,function(respones,bdy,tkn){
        ...
        var resData=writeSearchXmlAccountDeposit(respones,no_server_error,bdy);
           let newData= new newApiRequest.insertData(
             {
               rem_no :req.process_info.rem_no,
               mobile_no:req.process_info.posCode,
               ...
               responesData:JSON.stringify(respones),
               FirstName:(respones['CorporateName'] === undefined ?"":respones['CorporateName']),
               SecondName:"",
               ThirdName:"",
               LastName:"",
               CustID:"",
             ...
             newData.save( async (err,doc)=> {
               ...
               {
                 console.log(err);
                 var errResponse=writeSearchXmlAccountDeposit(respones,database_error,bdy);
        ...
        var Res_Code =(respones['ResultCode'] === undefined) ? -5 : respones['ResultCode']
           let newData= new newApiRequest.insertData(
             ...
             newData.save( async (err,doc)=> {
               ...
               {
                 console.log(err);
                 var errResponse=writePayXmlAccountDeposit(respones,database_error,bdy);
                 callback(errResponse);
               }
             });
           });
...
function mmPraperPayAccountDeposit(req, callback){
  ...
  newApiRequest.insertData.find({rem_no:decodeURI(req.process_info.rem_no)},(err,apiData)=>{
  ...
  posCode= apiData[apiData.length - 1].mobile_no;
}
else{
  TranId = "00000"
token = "no-token"
