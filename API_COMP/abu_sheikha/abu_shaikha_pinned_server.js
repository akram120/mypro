...
function Abu_sheikha_server(requestXmlFromInternalSystem,callback) {
    ...
    parser.parseString(requestXmlFromInternalSystem , function(err, result){
        ...
        pinnedRemitNo = remitNO;
        var builder = new xml2js.Builder();
        delete deletedObject.service_info;
        var xmlRequestToMusharbash = builder.buildObject(result);
        console.log("///////////////////////////////////////////////////////////////////////////");
        console.log(xmlRequestToMusharbash);
        

     if ((serviceInfo.service_type).toUpperCase() == "Q_REM") {
        getRemitDetails(xmlRequestToMusharbash).then(result => {
            ...
            if('ResponseCode' in ayserResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']){
                var yesResponseXML = builder.buildObject(result);
                console.log(remitNO);
                ...
                let newData = new newApiRequest.insertData(
                    {
                        ...
                        date: Date.now(),
                        requestData: xmlRequestToMusharbash,
                        responesData: yesResponseXML,
                        Amounts: "",
                        FirstName: "",
                        SecondName: "",
                        ThirdName: "",
        ...
        findQResponse(remitNO).then(value=> {
            payRemit(xmlRequestToMusharbash).then(result => {
...
module.exports.Abu_sheikha_server = Abu_sheikha_server;
...
async function getRemitDetails(xml) {
    ...
    var stringResultS = `<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
    <soap:Body>
       <GetRemitForPayResponse xmlns="http://tempuri.org/">
          <GetRemitForPayResult>
             <RemSendDate>13/09/2022</RemSendDate>
             <SenderName>test sender</SenderName>
             <SenderAddres>jordan</SenderAddres>
             <ReceiverName>test receiver</ReceiverName>
             <ReceiverPhone>123456789</ReceiverPhone>
             <ReceiverAddress>yemen</ReceiverAddress>
             <RemitCurrency>USD</RemitCurrency>
             <SenderCountry>JO</SenderCountry>
             <RemitAmount>1500</RemitAmount>
             <TransactionNo>${pinnedRemitNo}</TransactionNo>
             <PurposeDesc>Worker Transfers</PurposeDesc>
             <SettlementCurrency>USD</SettlementCurrency>
             <CommCurrency>USD</CommCurrency>
             <Commission>12.5</Commission>
             <SettlementAmount>15000</SettlementAmount>
          </GetRemitForPayResult>
       </GetRemitForPayResponse>
    </soap:Body>
 </soap:Envelope>`;

 var stringResultE = `<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
 <soap:Body>
    <GetRemitForPayResponse xmlns="http://tempuri.org/">
       <GetRemitForPayResult>
          <ResponseCode>65</ResponseCode>
          <ResponseMessage>Remit Suspended</ResponseMessage>
    ...
    var resultResponse;
    parser.parseString(stringResultS, function (err, result) {
...
async function payRemit(xml) {
    ...
    // var stringResult = body.toString();
    var stringResult =`<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
    <soap:Body>
       <PayRemitResponse xmlns="http://tempuri.org/">
          <PayRemitResult>
             <ResponseCode>0</ResponseCode>
             <ResponseMessage>Success</ResponseMessage>
             <StatusName>Remit Is Paid</StatusName>
             <TransactionNo>${pinnedRemitNo}</TransactionNo>
             <SettlementCurrency>USD</SettlementCurrency>
             <CommCurrency>USD</CommCurrency>
             <RemitCurrency>USD</RemitCurrency>
             <Commission>2</Commission>
             <RemitAmount>5</RemitAmount>
             <SettlementAmount>5</SettlementAmount>
             <PurposeDesc>Personal Transfer</PurposeDesc>
          </PayRemitResult>
       </PayRemitResponse>
    </soap:Body>
 </soap:Envelope>`;
