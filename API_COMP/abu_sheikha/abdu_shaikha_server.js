...
const url = 'https://api.hawalati.com:81/Services/WSRemitAPI.asmx?wsdl';
...
function Abu_sheikha_server(requestXmlFromInternalSystem,callback) {
    ...
    parser.parseString(requestXmlFromInternalSystem , function(err, result){
        ...
        var xmlRequestToAbduShiekha = builder.buildObject(result);
        console.log("///////////////////////////////////////////////////////////////////////////");
        console.log(xmlRequestToAbduShiekha);
        

     if ((serviceInfo.service_type).toUpperCase() == "Q_REM") {
        getRemitDetails(xmlRequestToAbduShiekha).then(result => {
            var hawalatiResponseJSON = result['soap:Envelope']['soap:Body'];
            var builder = new xml2js.Builder();
            if('ResponseCode' in hawalatiResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']){
                var hawalatiResponseXML = builder.buildObject(result);
                console.log(remitNO);
                var responseToInernalSystem = writeGeneralErrorQuery(hawalatiResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseMessage'],hawalatiResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseCode'],no_server_error);
                let newData = new newApiRequest.insertData(
                    {
                        ...
                        date: Date.now(),
                        requestData: xmlRequestToAbduShiekha,
                        responesData: hawalatiResponseXML,
                        Amounts: "",
                        FirstName: "",
                        SecondName: "",
                        ThirdName: "",
                ...
                newData.save(async (err, doc) => {
                    if (!err) {
                        ...
                        var responseToInernalSystemE = writeGeneralErrorQuery(hawalatiResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseMessage'],hawalatiResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult']['ResponseCode'],database_error);
                        callback(responseToInernalSystemE);
                    }
                });
            } else {
                    var detailedResponse = hawalatiResponseJSON['GetRemitForPayResponse']['GetRemitForPayResult'];
                    var hawalatiResponseXML = builder.buildObject(result);
        ...
        findQResponse(remitNO).then(value=> {
            ...
            payRemit(xmlRequestToAbduShiekha).then(result => {
                console.log("inside payRemit function")
                var hawalatiResponseJSON = result['soap:Envelope']['soap:Body']['PayRemitResponse']['PayRemitResult'];
                var builder = new xml2js.Builder();
                var hawalatiResponseXML = builder.buildObject(result);
                var responseToInernalSystem = writeConfirmPayResponse(hawalatiResponseJSON,no_server_error);
                let newData = new newApiRequest.insertData(
                    {
                        rem_no: remitNO,
                        service_name: serviceInfo.service_name,
                        ...
                        Request: requestXmlFromInternalSystem,
                        remStatus:hawalatiResponseJSON['ResponseCode']==0?"1":""
                    });
                console.log(newData);
                ...
                newData.save(async (err, doc) => {
                    if (!err) {
                        ...
                        console.log("DataBase Error");
                        var responseToInernalSystemE = writeConfirmPayResponse(hawalatiResponseJSON,database_error);
                        callback(responseToInernalSystemE);
                    }
                });
            }).catch(err=>{
...
async function payRemit(xml) {
            ...
            var stringResult = body.toString();
            console.log("result of stringResult is "+stringResult);
    var resultResponse;
    parser.parseString(stringResult, function (err, result) {
        
        console.log("result of respons is "+result);
        resultResponse = result;
    });
    return resultResponse;
    ...
    await newApiRequest.insertData.find({ rem_no: number }, (err, apiData) => {
        try {
            if (apiData[0].qRespones === undefined) {
                ...
                console.log("Qresponse is : " + entireRow);
                resolve(entireRow);
            }
