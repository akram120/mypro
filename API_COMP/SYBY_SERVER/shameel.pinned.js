...
const baseUrl = "https://shamilmoney.com:9305/SBYB/API/Agent/";
...
const password = 'Zvzika576';
const client_id = '1c74d00f-0560-4392-92f9-c3380eb4d763';
const client_secret = '6946ac5e-edfb-4ad0-b0cf-f22b5a304dbc';
const newApiRequest = require('../../db_modal/alnoamanNewModal');
const shamilTokenHeader = { 'content-type': TokenContent_type };
const tokenid = 'yGLNNoqKGkXvlfJAfndcMvCHXNflATSWhBavDWvcNPciZTqDgImrAQLWTryNIIgnlxroBE';
    ...
    getToken(function (status, pdata) {
      ...
      if (status == 200) {
        ...
        var depositeProofAmount = req.process_info.amount;
         depositeProofByName = req.rem_info.by_Name1;
  ...
  request.post(
    {
      ...
      url: "http://100.0.0.108:3150/shameel/login",
      body: body,
      method: 'POST',
    },
  ...
  request.post(
    {
      ...
      method: 'POST',
      agentOptions: {
        // cert:fs.readFileSync('C:/shameelServer/cert.pem'),
        // key:fs.readFileSync('C:/shameelServer/KEY.pem'),
    }
    },
    function (err, respones, body) {
...
function getWithdrawalReq(callback, tokenNo, transNo, mobileNo, amount, branchName, branchNo) {
  ...
  request.post(
    {
      headers: balanceHeader,
      url: "http://100.0.0.108:3150/shameel/cashOut",
      body: bodyRequest,
      method: 'POST',
    },
    function (err, respones, body) {
...
function getWithdrawal(callback, gottenToken, TransNo, mobileNo, amount, BranchName, BranchNo, OTP) {
  ...
  request.post(
    {
      headers: balanceHeader,
      url: "http://100.0.0.108:3150/shameel/cashOut-confirm",
      body: bodyRequest,
      method: 'POST',
    },
    function (err, respones, body) {
...
function getDepositReq(callback, tokenNo, prn, mobileNo, amountDeposit) {
  ...
  request.post(
    {
      headers: balanceHeader,
      url: "http://100.0.0.108:3150/shameel/cashIn",
      body: bodyRequest,
      method: 'POST',
    },
    function (err, respones, body) {
...
function getDeposit(callback, tokenNo, transNo, mobileNo, amount, by, branchName, branchNo) {
  ...
  request.post(
    {
      headers: balanceHeader,
      url: "http://100.0.0.108:3150/shameel/cashIn-confirm",
      body: bodyRequest,
      method: 'POST',
    },
    function (err, respones, body) {
...
function writeCashInCashOutQXmlFile(service, responesData, ServerData) {
  var x;
  if (service == "CASHIN_Q") {
    x = responesData.EXDATA.fees.fee_amt
